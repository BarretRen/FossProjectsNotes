...
- TCP_NODELAY已经开启，立刻发送
- unacked与cwnd接近，unacked的数量过多了，拥塞机制导致不再发送。并有重传导致cwnd重置
- 增大RTO超时时间，减少网络延迟导致的误判, 默认是`LWIP_TCP_RTO_TIMEOUT`

影响语音数据堆积的关键配置：

| 配置项            | 作用                 | 对语音的影响         |
| :---------------- | :------------------- | :------------------- |
| PBUF_POOL_SIZE    | 常规数据包缓冲区数量 | 直接影响并发处理能力 |
| TCP_SND_BUF       | TCP 发送缓冲区大小   | 影响数据传输速率     |
| TCP_WND           | TCP 接收窗口大小     | 影响接收端流量控制   |
| TCP_SND_BUF_COUNT | 发送缓冲区数量       | 影响并发发送能力     |

## tcp关闭socket后后台仍然发送缓冲区数据

受如下配置控制：

| SO_LINGER 配置      | 缓冲区有数据时 close 行为   | 是否发 FIN                       | 是否发 RST   |
| :------------------ | :-------------------------- | :------------------------------- | :----------- |
| l_onoff=0（默认）   | 等待全部数据发送 + ACK 完成 | 数据发完后才发 FIN               | 否           |
| l_onoff=1, linger>0 | 限时发送数据，超时丢数据    | 超时前发完则发 FIN，超时直接 RST | 超时触发     |
| l_onoff=1, linger=0 | 立刻丢弃所有缓冲数据        | 永不发送 FIN                     | 立即发送 RST |

代码中设置：

```c
#define LWIP_SO_LINGER   1 //必须打开

// 创建TCP socket并预先强制RST关闭配置
int create_force_rst_socket(void)
{
    ...
    l_opt.l_onoff  = 1;
    l_opt.l_linger = 0;
    if(lwip_setsockopt(fd, SOL_SOCKET, SO_LINGER, &l_opt, sizeof(l_opt)) != 0) {
        lwip_close(fd);
        return -1;
    }

    // 后续connect、send、recv逻辑...
    return fd;
}
```
...
- `TCP_TMR_INTERVAL`, 250 ms, TCP 基础定时器间隔
- `TCP_SLOW_INTERVAL`, 500 ms, 慢速定时器间隔（`2 * TCP_TMR_INTERVAL`）
- `LWIP_TCP_RTO_TIMEOUT`, 3000 ms
- `TCP_SYNMAXRTX`, 6, SYN 包最大重传次数
- `TCP_MAXRTX`, 12（默认）, 数据段最大重传次数

### SYN重传 - 固定间隔

SYN按照RTO的值固定间隔的重试, 直到达到最大重传次数:
...
| 第5次重传 | 15s    | 3s（固定）           |
| 第6次重传 | 18s    | 3s（固定）           |
| 放弃连接  | 21s    | 达到 TCP_SYNMAXRTX=6 |

### 数据重传 - 指数退避

根据`tcp_backoff`配置进行指数退避:

| 重传次数(nrtx) | backoff值 | RTO  | 实际间隔       |
| -------------- | --------- | ---- | -------------- |
| 1              | 1         | 2s   | 2s             |
| 2              | 2         | 4s   | 4s             |
| 3              | 3         | 8s   | 8s             |
